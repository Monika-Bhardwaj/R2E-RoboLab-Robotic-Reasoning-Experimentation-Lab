...
class R2EEnv:
    def __init__(self):
        ...
        self._known_variables: dict[str, str] = {}
        self._done: bool = False
        self._steps: list[StepRecord] = []
        self._reasoning_scores: list[float] = []

    async def reset(self, task: str = "easy", seed: int = 42) -> Observation:
    ...
    async def step(self, action: R2EAction) -> tuple[Observation, float, bool, dict]:
        ...
        next_obs, rc = self._dynamics.apply_action(
            ...
            hidden=self._hidden,
            obs=self._obs,
            revealed_probes=self._revealed_probes,
            known_variables=self._known_variables,
        ...
        self._obs = next_obs
        self._reasoning_scores.append(rc.reasoning_quality)
        ...
        return (next_obs, reward, done, {"reason": reason, "reasoning_quality": rc.reasoning_quality})
    ...
    def state(self) -> dict:
        return {
            ...
            "done": self._done,
            "phase": self._obs.phase if self._obs else "investigation",
            "revealed_probes": list(self._revealed_probes),
            "known_variables": dict(self._known_variables),
    ...
    def get_episode_log(self) -> EpisodeLog | None:
        ...
        return EpisodeLog(
            ...
            reasoning_scores=list(self._reasoning_scores),
        )
